...
export class UserController {
  ...
  async patchUser(req, res) {
    ...
    if (this.checkSelf(requestingUserID, targetUser.uid)) {
      try {
        ...
        if (false === _.isUndefined(req.body.express)) {
          targetUser.set({ express: req.body.express });
        }
    ...
    if (this.checkSelf(requestingUserID, targetUser.uid)) {
      try {
        if (false === _.isUndefined(req.body.nickname)) {
          // nickname
          if (false === await this.isNicknameAvailable(req.body.nickname)) {
        ...
        targetUser.set({ reliability: step * 10, init_step: step });
  ...
  async checkNicknameAvailability(req, res) {
    ...
    try {
      if (await this.isNicknameAvailable(nickname)) {
