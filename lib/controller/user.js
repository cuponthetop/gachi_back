...
export class UserController {
  ...
  async getUserInformation(req, res) {
    ...
    let leadrooms = [];
    try {
      history = this.userMgr.model.findAll({
        where: {
          member: requestingUserID,
          $lt: {
            '$leadroom.lead_until$': Date.now()
          }
        },
        include: [{
    ...
    try {
      leadrooms = this.userMgr.model.findAll({
        where: {
          member: requestingUserID,
          $gt: {
            '$leadroom.lead_from$': Date.now()
          }
        },
        include: [{
          model: this.leadroomMgr.model,
    ...
    try {
      ...
      return succeedWith(logger, res, res, 200);
    } catch (e) {
      return failWith(logger, res, e.message, 500);
    }
  }
    ...
    if (this.checkSelf(requestingUserID, targetUser.uid)) {
      try {
        ...
        if (false === _.isUndefined(req.body.profile_image)) {
          let image = null;
          if (false === _.isNull(req.body.profile_image)) {
            image = await retrieveImage(req.body.profile_image);
          }
          // image
          targetUser.set({ profile_image: image });
        }
        if (false === _.isUndefined(req.body.nickname)) {
          // nickname
  ...
  async initUser(req, res) {
    ...
    if (this.checkSelf(requestingUserID, targetUser.uid)) {
      try {
        ...
        targetUser.set({ reliability: prevRel + 10 });
