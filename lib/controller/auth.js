...
export class AuthController {
  ...
  async authenticate(req, res, next) {
    if (_.isUndefined(req.header.access_token)) {
      return failWith(logger, res, 'Required Field req.header.access_token not found', 400);
    }

    try {
      let fbUserInfo = await this.fbApp.verifyIDToken(req.header.access_token);
