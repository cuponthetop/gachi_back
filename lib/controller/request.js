...
export class RequestController {
  ...
  async getRequestInfo(req, res) {
    ...
    try {
      let leadroom = request.getLeadroom();
      let requester = request.getRequester();

      request.leadroom = leadroom;
      request.requester = requester;

      return succeedWith(logger, res, request, 200);
    } catch (e) {
  ...
  async acceptRequest(req, res) {
    ...
    try {
      ...
      membersRef.push({ uid: requester.uid, fbid: requester.fbid, username: requester.nickname });
