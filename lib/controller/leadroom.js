...
async function includeMembersToResults(result) {
  let resultsPromises = _.map(result, async function(el) {
    return await includeMember(el, el.toJSON());
  });
...
export class LeadroomController {
  ...
  async getLeadroomList(req, res) {
    ...
    try {
      ...
      result = await includeMembersToResults(result);
  ...
  async getLeadroom(req, res) {
    ...
    let members = [];
    try {
      let result = await includeMember(leadroom, leadroom.toJSON());
