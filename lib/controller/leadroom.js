...
import { includeAllForLeadroom, includeAllForLeadroomToResults } from '../util/leadroom-formatter';
...
export class LeadroomController {
  ...
  async getLeadroomList(req, res) {
    ...
    try {
      ...
      result = await includeAllForLeadroomToResults(result);
  ...
  async getLeadroom(req, res) {
    ...
    let members = [];
    try {
      let result = await includeAllForLeadroom(leadroom, leadroom.toJSON());
  ...
  async createLeadroom(req, res) {
    ...
    try {
      ...
      let membersRef = chatRef.child("members/" + fbid);

      await msgRef.push({ userid: 'system', username: 'system', text: '채팅방을 생성하였습니다.', timestamp: Date.now() });
      await membersRef.set({ uid, username: userInfo.nickname });
  ...
  async updateLeadroom(req, res) {
    ...
    let title = _.get(req.body, 'title', leadroom.title);
  ...
  async exitLeadroom(req, res) {
    ...
    try {
      ...
      let member = await membersRef.orderByKey().equalTo(fbid).once('value');
      await member.remove();
