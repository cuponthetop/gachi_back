...
async function includeLeadrooms(festival, jsoned) {
  let leadrooms = await festival.getLeadrooms();
  _.set(jsoned, 'leadrooms', _.map(leadrooms, function (leadroom) { return { leadroom: leadroom }; }));
  return jsoned;
}

async function includeGenres(festival, jsoned) {
  let genres = await festival.getGenres();
  _.set(jsoned, 'genres', _.map(genres, function (genre) { return { genre: genre.genre }; }));
  return jsoned;
}

async function includeGenresToResults(result) {
  let resultsPromises = _.map(result, async function (el) {
    return await includeGenres(el, el.toJSON());
  });
...
export class FestivalController {
  ...
  async getFestivalList(req, res) {
    ...
    try {
      ...
      result = await includeGenresToResults(result);
  ...
  async getFestival(req, res) {
    ...
    try {
      ...
      let result = festival.toJSON();

      result = await includeGenres(festival, result);
      result = await includeLeadrooms(festival, result);
