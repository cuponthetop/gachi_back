...
export async function handlerServiceBuilder(args) {
  ...
  let storageManager = _.get(args, 'storageManager', null);
  ...
  _.assign(ret, { authCtrl: new AuthController(firebaseService, userManager) });
  ...
  _.assign(ret, { leadroomCtrl: new LeadroomController(userManager, festivalManager, leadroomManager, leadroomMemberManager, firebaseService) });
  _.assign(ret, { userCtrl: new UserController(userManager, leadroomManager, leadroomMemberManager) });
  _.assign(ret, { festivalCtrl: new FestivalController(festivalManager, leadroomManager, leadroomMemberManager, festivalGenreManager, storageManager) });
